...
process_by = function(con, .by = 16, .geoid = '36109', .level = "county", .run = NULL, .pollutant = NULL){
  ...
  # LOAD METADATA!
  data("metadata", overwrite = TRUE, envir=environment());# metadata = metadata[[.level]]
  
  # Get formatting metadata for an emissions dataset corresonding to your supplied .level of moves data. 
  .m = metadata[[.level]]$emissions
  ...
  q_e = q_e %>% select(!!.m$labels) #select(!!.m$vars)
  ...
  # Get formatting metadata for an activity dataset corresonding to your supplied .level of moves data. 
  .m = metadata[[.level]]$activity
  # Access movesoutput table (emissions)
  q_a = con %>% tbl("movesactivityoutput") 
  # If there's a specific run, filter to that run (but it's not required)
  if(!is.null(.run)){ q_a = q_a %>% filter(MOVESRunID == !!.run)   }
  # Grab and rename columns according to our template .m
  q_a = q_a %>% select(!!.m$labels) # select(!!.m$vars)
  ...
  # Make .by an integer, since that's more efficient for our Cloud SQL table
  .by = as.integer(.by)
  result = emissions %>% 
